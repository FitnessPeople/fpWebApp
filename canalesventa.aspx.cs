...
namespace fpWebApp
{
    public partial class canalesventa : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    ValidarPermisos("Canales de ventas");
                    ...
                    ListaCanalesDeVenta();
                    ltTitulo.Text = "Agregar Canal de venta";

                    if (Request.QueryString.Count > 0)
                    {
                        rpCanalesVenta.Visible = false;
                        if (Request.QueryString["editid"] != null)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                txbNombreCanalVenta.Text = dt.Rows[0]["NombreCanalVenta"].ToString();                               
                        ...
                        if (Request.QueryString["deleteid"] != null)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                if (dt1.Rows.Count > 0)
                                {
                                    txbNombreCanalVenta.Text = dt1.Rows[0]["NombreCanalVenta"].ToString();
                                    txbNombreCanalVenta.Enabled = false;
                                  
                                    btnAgregar.Text = "⚠ Confirmar borrado ❗";
                                    btnAgregar.Enabled = false;
                                    ltTitulo.Text = "Borrar Canal de venta";
                                }
                                dt1.Dispose();
                            }
                            else
        ...
        private void ListaCanalesDeVenta()
        {
            ...
            rpCanalesVenta.DataSource = dt;
            rpCanalesVenta.DataBind();
            dt.Dispose();
        }


        protected void rpCanalesVenta_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                if (ViewState["CrearModificar"].ToString() == "1")
                {
                    ...
                    btnEditar.Attributes.Add("href", "canalesventa?editid=" + ((DataRowView)e.Item.DataItem).Row[0].ToString());
                    btnEditar.Visible = true;
                }
                if (ViewState["Borrar"].ToString() == "1")
                {
                    HtmlAnchor btnEliminar = (HtmlAnchor)e.Item.FindControl("btnEliminar");
                    btnEliminar.Attributes.Add("href", "canalesventa?deleteid=" + ((DataRowView)e.Item.DataItem).Row[0].ToString());
                    btnEliminar.Visible = true;
                }
            }
        }

        private bool ValidarCanalVenta(string strNombre)
        {
            bool bExiste = false;
            clasesglobales cg = new clasesglobales();
            DataTable dt = cg.ConsultarCanalesVentaPorNombre(strNombre);
            if (dt.Rows.Count > 0)
            {
                bExiste = true;
            }
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString.Count > 0)
            {
                ...
                if (Request.QueryString["deleteid"] != null)
                {
                    string respuesta = cg.EliminarCanalVenta(int.Parse(Request.QueryString["deleteid"].ToString()));
                }
                Response.Redirect("canalesventa");
            }
            else
            {
                if (!ValidarCanalVenta(txbNombreCanalVenta.Text.ToString()))
                {
                    try
                    {
