    ...
    public partial class imprimirafiliados : System.Web.UI.Page
    {
        ...
        protected void lbExportarExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // TODO: Falta el filtro del Buscador
                string consultaSQL = @"SELECT DocumentoAfiliado AS 'Nro. de Documento', CONCAT_WS(' ', NombreAfiliado, ApellidoAfiliado) AS 'Nombre', 
                                       CelularAfiliado AS 'Celular', EmailAfiliado AS 'Correo', DireccionAfiliado AS 'Dirección de Residecia', 
                                       FechaNacAfiliado AS 'Fecha de Nacimiento', TIMESTAMPDIFF(YEAR, FechaNacAfiliado, CURDATE()) AS 'Edad', 
                                       FechaAfiliacion AS 'Fecha de Afiliación', s.NombreSede AS 'Sede', EstadoAfiliado AS 'Estado'
                                       FROM Afiliados a
                                       INNER JOIN Sedes s ON s.IdSede = a.idSede
                                       WHERE a.idSede = " + ddlSedes.SelectedItem.Value.ToString() + " " + strQueryAdd;
